...
This page is the detailed technical reference for EPM equations. For a prose description of the model, see [Model Overview](model_overview.md). For the LaTeX formulation of the objective function and main constraints, see [Mathematical Formulation](model_formulation.md).
...
## Dispatch Mode Equations

When `fDispatchMode=1`, EPM runs the full hourly chronology instead of representative days. This activates three additional blocks of equations. See [Dispatch Mode](../run/run_dispatch.md) for how to enable these features.

### Dispatch continuity and ramping

Ensures that hourly generation `vPwrOut` and the slot-aggregated variable `vPwrOutDispatch` are consistent at every time step, and that slot-to-slot ramp changes respect physical ramp rates.
...
Slot-to-slot ramp limits (`eDispatchRampUpLimit`, `eDispatchRampDownLimit`) operate alongside the existing hourly ramp equations (`eRampUpLimit`, `eRampDnLimit`). Storage SOC transitions (`eStorageHourTransition`, `eStorageDayWrap`) also operate over the full chronology, so generation and storage trajectories are traced continuously.

!!! note
    CSP and hydrogen modules do not yet have equivalent full-hour continuity enforcement and will be updated in a future version.

### Unit commitment

Generators with a `MinGenPoint` activate binary variables `isOn`, `isStartup`, and `isShutdown`. These are linked to dispatch output and to the objective function via startup costs.
...
The minimum generation constraint forces any positive output to raise the commitment flag; the maximum generation constraint caps output when `isOn=0`:
...
`eStartupCostConstraint` counts startups per slot and feeds `vYearlyStartupCost` in the objective. Minimum up/down time windows are enforced by the `MinUp...`/`MinDown...` rolling constraints when `fApplyMUDT=1`.

### Required flag combinations

| Feature | Required flags |
|---|---|
| Dispatch continuity + ramp | `fDispatchMode=1`, `fRampConstraints=1`, `fApplyRampConstraint=1` |
| Unit commitment + startup cost | above + `fApplyMinGenerationConstraint=1`, `fApplyStartupCost=1` |
| Minimum up/down time | above + `fApplyMUDT=1` |
