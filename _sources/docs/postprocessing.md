...
- **`standardize_names`**: Standardizes fuel names in results using a predefined mapping.
- **`filter_dataframe`**: Filters a DataFrame based on a dictionary of conditions.
- **`filter_dataframe_by_index`**: Filters a DataFrame by conditions applied to the index levels.
- **`process_epm_inputs`**: Processes EPM input files for plotting.
- **`process_epm_results`**: Processes EPM output results for plotting.
- **`process_simulation_results`**: Extracts and processes both inputs and outputs.
