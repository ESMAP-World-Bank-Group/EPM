...
***************************************************************************
 pMinREAnnual(y) Min RE share per year
...
$onecho > gdxxrw.in
par=ftfindex                rdim=2 cdim=0 rng=FuelTechnologies!A3:A33
...
par=pTechDataExcel          rdim=1 cdim=1 rng=FuelTechnologies!B35:F59
...
par=pMaxFuellimitDay           rdim=2 cdim=1 rng=FuelLimitDay!A6
par=pMinREAnnual                    rDim=1      rng=MinRE!A6
...
option cfmap<pMaxFuellimitDay, Error01<cfmap;
Error01(c,f)$(Error01(c,f)=1) = 0;
put 'Validating fuel mappings [Fuel Limits] ...';
if(card(Error01),
  put / 'pMaxFuellimitDay:: More than one fuel map per country!'/;
  put / 'SYMBOL' @25 'COUNTRY' @42 'FUEL';
  loop((c,f)$(Error01(c,f) > 0),
      put / 'pMaxFuellimitDay:: ' @25 c.tl @42 f.tl;
    );
  abort 'More than one fuel map per country [Fuel Limits]', Error01;
else
  put ' OK'/;
...
***************New***********************
pAvailabilityY(g,q,y)=pAvailability(g,q);
pAvailabilityY(gc,q,y)$(ord(y)=2)=0.65;
pAvailabilityY(gc,q,y)$(ord(y)=3)=0.7;
pAvailabilityY(gc,q,y)$(ord(y)>3)=0.75;
...
vStorInj.fx(g,q,d,t,y)$(sum(z,pDemandData(z,q,d,y,t))=0)=0;
...
display pRR, pWeightYear, pGenData, pCapacityCredit, pMinREAnnual,pMaxFuelLimit, pMaxFuelLimitDay,zExt, pTradePrice, pallowexports, pMaxExchangeShare, sImportPrice,pMaxImport,pMaxExport, GASALL, LNGALL;
