...
file
Option
limRow = 0, limCol = 0, sysOut = off, solPrint = off;
...
$set
BASE_FILE
"WB_EPM_v8_5_base.gms"
$endIf
$log
BASE_FILE is "%BASE_FILE%"
...
$set
REPORT_FILE
"WB_EPM_v8_5_Report.gms"
$endIf
$log
REPORT_FILE is "%REPORT_FILE%"
...
'technologies'
gstatus
'generator status' / Existing, Candidate, Committed /
techhdr
'techdata headers' / 'Construction Period (years)', 'RE Technology', 'Hourly Variation' /
pe
'peak energy for demand forecast' / 'peak', 'energy' /
ft
'fuel types'
sc
'settings' /
...
addition
H2status
'H2 generation plant status' / Existing, Candidate, Committed /
...
Set
ftfmap(ft <, f <)
'map fuel types to fuels'
zcmap(z <, c <)
'map zones to countries'
sTopology(z, z2)
'network topology - to be assigned through network data'
Peak(t)
'peak period hours'
Relevant(d)
'relevant day and hours when MinGen limit is applied'
mipopt(mipline <) / system.empty /;
;
Parameter
pCSPData(g, csphrd, shdr)
pCapexTrajectory(tech, y)
'Capex trajectory'
pTechData(tech <, techhdr)
'Technology data'
pFuelTypeCarbonContent(ft)
'Fuel type carbon content in tCO2 per MMBTu'
pStorDataInput(g, g2, shdr)
'Storage data'
pNewTransmission(z, z2, thdr)
'new transmission lines'
pTradePrice(zext, q, d, y, t)
'trade price - export or import driven by prices [assuming each zone in a country can only trade with one external zone]'
pMaxExchangeShare(y, c)
'Max share of exchanges by country [same limit for imports or exports for now]'
pDemandProfile(z, q, d, t)
'Demand profile in per unit'
pDemandForecast(z, pe, y)
'Peak and Energy demand forecast in MW and GWh'
pDemandData(z, q, d, y, t)
'hourly load curves by quarter(seasonal) and year'
pEmissionsCountry(c, y)
'Maximum zonal emissions allowed per country and year in tns'
pEmissionsTotal(y)
'Maximum total emissions allowed per year for the region in tns'
pCarbonPrice(y)
'Carbon price in USD per ton of CO2eq'
$ifi % mode %==MIRO
pHours(q <, d <, y <, t <)
...
pLossFactor(z, z2, y)
'loss factor in percentage'
pVREProfile(z, *, q, d, t)
'VRE generation profile by site quarter day type and YEAR -- normalized (per MW of solar and wind capacity)'
pVREgenProfile(g, f, q, d, t)
'VRE generation profile by plant quarter day type and YEAR -- normalized (per MW of solar and wind capacity)'
pAvailability(g, q)
'Availability by generation type and season or quarter in percentage - need to reflect maintenance'
pSpinningReserveReqCountry(c, y)
'Spinning reserve requirement local at country level (MW)  -- for isolated system operation scenarios'
pSpinningReserveReqSystem(y)
'Spinning reserve requirement systemwide (MW) -- for integrated system operation scenarios'
pScalars(sc)
'Flags and penalties to load'
pPlanningReserveMargin(c)
'Country planning reserve margin'
pEnergyEfficiencyFactor(z, y)
'Scaling factor for energy efficiency measures'
pExtTransferLimit(z, zext, q, *, y)
'transfer limits by quarter (seasonal) and year with external zones'
...
parameters ** ** ** ** ** ** ** ** ** ** ** *
pH2Data(hh, hhdr)
'H2 production unit specs'
pAvailabilityH2(hh, q)
'Availability by H2 generation plant and season or quarter in percentage - need to reflect maintenance'
pFuelData(f)
'Hydrogen fuels'
pCapexTrajectoryH2(hh, y)
'CAPEX trajectory for hydrogen generation unit'
** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** *

;
Scalar
solverThreads
'Number of threads available to the solvers' / 1 /
solverOptCR
'Relative gap for MIP Solver' / 0.05 /
solverResLim
...
Set
NLPSolver
'Selected NLP Solver' / '' 'conopt4' /
MIPSolver
'Selected MIP Solver' / '' 'cplex' /
;
$ifI % mode %==MIRO
$offExternalInput

Parameters
pCapexTrajectories(g, y)
'capex trajectories for all generators (used in results)'
pAllHours(q, d, y, t)
'Hour of system peak'
pFuelPrice(c, f, y)
'Fuel price forecasts by country'
pFindSysPeak(y)
'System peak per year'
pEmissionsTotal(y)
'Maximum total emissions allowed per year for the region in tns'
pSeasonalReporting
'seasonal reporting flag'
pSystemResultReporting
'system reporting file flag'
pInterConMode
'interconnected mode flag'
pNoTransferLim
'transfer limit flag'
pAllowExports
'Allow price based exports'
pMaxHrImportShare
pVRECapacityCredits
'User input capacity credits'
pDR
'discount rate'
pCaptraj
'allow capex trajectory'
pIncludeEE
'energy efficiency factor flag'
pSystem_CO2_constraints
pExtTransferLimitIn(z, zext, q, y)
'transfer limit with external zone for import towards internal zone'
pExtTransferLimitOut(z, zext, q, y)
'transfer limit with external zone for export towards external zone'
pMaxLoadFractionCCCalc
'maximum percentage difference between hourly load and peak load to consider in the capacity credit calculation'
pVREForecastError
'Percentage error in VRE forecast [used to estimated required amount of spinning reserve]'

;

Set
gprimf(g, f)
'primary fuel f for generator g'
tech / ROR
'Run of river hydro'
CSP
...
gtechmap(g, tech)
'Generator technology map'
gstatusmap(g, gstatus)
...
Sets
ghdr
'Additional headers for pGenData' / CapacityCredit, Heatrate, Heatrate2, Life, VOM /
shdr
'Additional headers for pStorData' / Life, VOM /
thdr
'Additional header for pNewTransmission' / EarliestEntry /
;
$offmulti
...
MIPSolver.te
Option
NLP = % NLPSOLVER %, MIP = % MIPSOLVER %, threads = % SOLVERTHREADS %, optCR = % SOLVEROPTCR %, resLim = %SOLVERRESLIM %;
...
os.environ['MYDATE'] = datetime.datetime.now().strftime(
    "%Y_%m_%d")  # we can add hour, minute, or seconds if necessary: %H %M %S
$  offEmbeddedCode
$  set
XLS_OUTPUT % fp %% fn % _ % sysenv.MYDATE %% fe %
$endIf.timestamp
...
display
tech, hh, pH2DataExcel, g, pFuelData, pDemandData, pExternalH2;
...
set
addHdr / fuel1, fuel2, Zone, Type, 'Assigned Value', status, Heatrate2,
...
Error01(c, f);
option
cfmap < pFuelPrice, Error01 < cfmap;
Error01(c, f)$(Error01(c, f)=1) = 0;
log.ap = 1;
put
log
put
'Validating fuel mappings [Fuel Prices] ...';
if (card(Error01),
...
option
cfmap < pMaxFuellimit, Error01 < cfmap;
Error01(c, f)$(Error01(c, f)=1) = 0;
put
'Validating fuel mappings [Fuel Limits] ...';
if (card(Error01),
...
put
'Validating duration of each block data ...';
if (card(Error04),
...
put
'Validating VRE generation profile by site data ...';
if (card(Error05),
...
Error06(g, q);
*option
Error06 < pAvailability;
*Error06(g, q)$(pAvailability(g, q) < 1.01) = 0;
*put
'Validating generator availability data ...';
* if (card(Error06),
      ...
      * loop(Error06(g, q),
*put / 'pAvailability:: ' @ 25
g.tl @ 45
q.tl @ 65
pAvailability(g, q);
* );
*abort
'Generator availability factor cannot be greater than 1', Error06;
* else
*put
' OK' /;
* );
...
getDivisor(z, q, d, y, t)..pyval(z, q, d, y, t) = e = [2 * pdiff(z, y) / divisor(z, y) * (
            pmax(z, y) - pDemandProfile(z, q, d, t))] / sqr(pmax(z, y) - pmin(z, y));
getArea(z, y)..sum((q, d, t), pyval(z, q, d, y, t) * pHours(q, d, y, t)) = e = pdiff(z, y);
objFn..obj = e = sum((z, q, d, y, t), pyval(z, q, d, y, t));

model
demand / getDivisor, getArea, objFn /;
...
parameter
initialisation
for same demand profile for all years
...
fuel_constraints - 1
$set
capital_constraints - 1
$set
mingen_constraints - 1
$set
includeCSP - 1
$set
includeStorage - 1
$set
zonal_co2_constraints - 1
$set
system_co2_constraints - 1
...
NOT
"%zonal_spinning_reserve_constraints%" == "-1"
pzonal_spinning_reserve_constraints = %zonal_spinning_reserve_constraints %;
$IF
NOT
"%system_spinning_reserve_constraints%" == "-1"
psystem_spinning_reserve_constraints = %system_spinning_reserve_constraints %;
$IF
NOT
"%planning_reserve_constraints%" == "-1"
pplanning_reserve_constraints = %planning_reserve_constraints %;
$IF
NOT
"%ramp_constraints%" == "-1"
pramp_constraints = %ramp_constraints %;
$IF
NOT
"%fuel_constraints%" == "-1"
pfuel_constraints = %fuel_constraints %;
$IF
NOT
"%capital_constraints%" == "-1"
pcapital_constraints = %capital_constraints %;
$IF
NOT
"%mingen_constraints%" == "-1"
pmingen_constraints = %mingen_constraints %;
$IF
NOT
"%includeCSP%" == "-1"
pincludeCSP = %includeCSP %;
$IF
NOT
"%includeStorage%" == "-1"
pincludeStorage = %includeStorage %;
$IF
NOT
"%zonal_co2_constraints%" == "-1"
pzonal_co2_constraints = %zonal_co2_constraints %;
$IF
NOT
"%system_co2_constraints%" == "-1"
psystem_co2_constraints = %system_co2_constraints %;

$IF
NOT
"%IncludeDecomCom%" == "-1"
pIncludeDecomCom = %IncludeDecomCom %;
** ** ** ** ** ** ** ** ** *Hydroge
model
parameters ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** *
$IF
...
if (pScalars("altDemand")
= 1, put 'Ignoring detailed demand and demand forecast data. Using same demand profile for all years instead.' /);
...
REH2(g) = sum(gtechmap(g, tech)$pTechData(tech, 'RE Technology'), 1) - sum(gtechmap(g, tech)$pTechData(tech,
                                                                                                       'Hourly Variation'), 1);
nREH2(g) = not REH2(g);
** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** **
...
$offIDCProtect
pVREgenProfile(gfmap(VRE, f), q, d, t)$(not (pVREgenProfile(VRE, f, q, d, t))) = sum(gzmap(VRE, z),
                                                                                     pVREProfile(z, f, q, d, t));
$onIDCProtect
pCapacityCredit(VRE, y)$(pVRECapacityCredits =1) = pGenData(VRE, "CapacityCredit");
pCapacityCredit(VRE, y)$(pVRECapacityCredits =0) = Sum((z, q, d, t)$gzmap(VRE, z), Sum(f$gfmap(VRE, f), pVREgenProfile(
    VRE, f, q, d, t)) *pAllHours(q, d, y, t)) *(Sum((z, f, q, d, t)$(gfmap(VRE, f) and gzmap(VRE, z) ), pVREgenProfile(
    VRE, f, q, d, t)) / sum((q, d, t), 1));
pCapacityCredit(ROR, y) = sum(q, pAvailability(ROR, q) * sum((d, t), pHours(q, d, y, t))) / sum((q, d, t),
                                                                                                pHours(q, d, y, t));
*pCapacityCredit(RE, y) = Sum((z, q, d, t)$gzmap(RE, z), Sum(f$gfmap(RE, f), pREProfile(z, f, q, d, t)) *pAllHours(q, d,
                                                                                                                   y,
                                                                                                                   t)) *(
    Sum((z, f, q, d, t)$(gfmap(RE, f) and gzmap(RE, z) ), pREProfile(z, f, q, d, t)) / sum((q, d, t), 1));
...
*execute_unload
'debug_output.gdx', g, gtechmap, pCapexTrajectory;

*pCapexTrajectories(dc, y)$pCaptraj = sum(tech$(gtechmap(g, tech)), pCapexTrajectory(tech, y));
pCapexTrajectories(dc, y)$pCaptraj = sum(tech$(pCapexTrajectory(tech, y) > 0 and gtechmap(dc, tech)), pCapexTrajectory(
    tech, y));
...
pRR(y) = 1.0;
pRR(y)$(ord(y) > 1) = 1 / ((1 + pDR) ** (sum(y2$(ord(y2) < ord(y)), pWeightYear(y2))-1 + sum(sameas(y2, y),
                                                                                             pWeightYear(y2) / 2)));
...
pCRFcth(cs)$pCSPData(cs, "Thermal Field", "Life") = pWACC / (
            1 - (1 / ((1 + pWACC) ** pCSPData(cs, "Thermal Field", "Life"))));
...
*Zones_conn_in_Diff_country(z, zz)$((sum(c$gfmap(c, z), x(c, z))
ne
sum(c$gfmap(c, zz), x(c, zz))) and conn(z, zz)) = 1;
...
vCap.fx(eg, y)$((pGenData(eg, "StYr") <= y.val) and (pGenData(eg, "StYr") >= sStartYear.val)) = pGenData(eg,
                                                                                                         "Capacity");
...
vCapStor.fx(eg, sStartYear)$(pGenData(eg, "StYr") < sStartYear.val) = pCSPData(eg, "Storage", "Capacity") + pStorData(
    eg, "Capacity");
...
vCapStor.fx(eg, y)$((pScalars("econRetire") = 0 and pGenData(eg, "StYr") < y.val) and (
            pGenData(eg, "RetrYr") >= y.val)) = pStorData(eg, "Capacity");
...
vCap.fx(eg, y)$((pScalars("econRetire") = 0 and pGenData(eg, "StYr") < y.val) and (
            pGenData(eg, "RetrYr") >= y.val)) = pGenData(eg, "Capacity");
...
new
capacity
for new generators can not exceed the annual limit
...
at
days
with zero demand (applicable to dispatch model)
...
power
drawn
for H2 production can't be more than the Capacity of the electrolyzer
...
vCapH2.fx(eh, y)$((pScalars("econRetire") = 0 and pH2Data(eh, "StYr") < y.val) and (
            pH2Data(eh, "RetrYr") >= y.val)) = pH2Data(eh, "Capacity");
...
not allow
imports and exports
for a zone without import / export prices
...
sSpinningReserve(g, q, d, t, y)$(
    not (pzonal_spinning_reserve_constraints or psystem_spinning_reserve_constraints)) = no;
