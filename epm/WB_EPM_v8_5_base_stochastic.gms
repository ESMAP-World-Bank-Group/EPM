...
Sets
   ...
   sExportPrice(z,zext,q,d,t,y,s)     'valid tuples for vExportPrice'
   sImportPrice(z,zext,q,d,t,y,s)     'valid tuples for vImportPrice'
   sAdditionalTransfer(z,z2,y) 'valid tuples for vAdditionalTransfer'
   sFlow(z,z2,q,d,t,y,s)         'valid tuples for vFlow'
   sSpinningReserve(g,q,d,t,y,s)         'valid tuples for vSpinningReserve'
************************H2 related set***************************************
   sH2PwrIn(hh,q,d,t,y)
;
...
Positive Variables
   ...
   vImportPrice(z,zext,q,d,t,y,s)   'external price driven import'
   vExportPrice(z,zext,q,d,t,y,s)   'external price driven export'
   ...
   vSpinningReserve(g,q,d,t,y,s)                 'Spinning reserve provision (MW)'
   vUnmetPlanningReserveCountry(c,y,s)              'Unserved zonal planning reserve'
   vUnmetPlanningReserveSystem(y)              'Unmet reserve for system -- capacity reserve'
   vUnmetSpinningReserveCountry(c,q,d,t,y,s)        'Unmet spinning reserve - local constraint'
   vUnmetSpinningReserveSystem(q,d,t,y,s)        'Unmet spinning reserve - system constraint'
...
Equations
   ...
   eSpinningReserveLim(g,q,d,t,y,s)              'Reserve limit as a share of capacity'
*eSpinningReserveLimVRE(g,q,d,t,y)           'Reserve limit for VRE as a share of capacity adjusted for production profile'
   ...
   eSpinningReserveReqCountry(c,q,d,t,y,s)          'Country spinning reserve requirement'
   eSpinningReserveReqSystem(q,d,t,y,s)          'System spinning reserve requirement'
   ePlanningReserveReqSystem(y)                'Min system planning reserve requirement'
   ePlanningReserveReqCountry(c,y,s)                'Minimum capacity reserve over peak demand at country level'

   eTransferLimit(z,z2,q,d,t,y,s)    'Transfer limits'
   eVREProfile(g,f,z,q,d,t,y,s)      'VRE generation restricted to VRE profile'
   eMaxImportPrice(c,y,s)            'import limits: max import from external zones'
   eMaxExportPrice(c,y,s)            'export limits: max export to external zones'
   eYearlySurplusCost(z,y)
   eAdditionalTransfer(z,z2,y)
   eAdditionalTransfer2(z,z2,y)
   eYearlyTransmissionAdditions (z,y)
   eExtZoneLimitImport(z,zext,q,d,t,y,s) 'import limits from external zone in MW'
   eExtZoneLimitExport(z,zext,q,d,t,y,s) 'export limits to external zone in MW'
...
eFuelLimitH2(c,f,y,s)
eFuelLimitH2_2(c,f,y,s)
eAnnCapexH2_1(hh,y)
eAnnCapexH2(hh,y)
...
*---    Objective function
eNPVCost..
   vNPVCost =e= sum(y, pRR(y)*pWeightYear(y)*(sum(c, vYearlyTotalCost(c,y)) + vYearlyUnmetReserveCostSystem(y)+sum(s, vYearlySysCO2backstop(y,s)*pProbaScenarios(s)*pCostOfCO2backstop)));
...
eYearlySpinningReserveCost(z,y)..
   vYearlySpinningReserveCost(z,y) =e= sum((gzmap(g,z),q,d,t,s), vSpinningReserve(g,q,d,t,y,s)*pProbaScenarios(s)*pGenData(g,"ReserveCost")*pHours(q,d,t));
...
eYearlyTradeCost(z,y)..
   vYearlyTradeCost(z,y) =e= sum((zext,q,d,t,s), vImportPrice(z,zext,q,d,t,y,s)*pProbaScenarios(s)*pTradePrice(zext,q,d,y,t)*pHours(q,d,t))
                           - sum((zext,q,d,t,s), vExportPrice(z,zext,q,d,t,y,s)*pProbaScenarios(s)*pTradePrice(zext,q,d,y,t)*pHours(q,d,t));

eYearlyUnmetReserveCostCountry(c,y)..
   vYearlyUnmetReserveCostCountry(c,y) =e= sum(s, vUnmetPlanningReserveCountry(c,y,s)*pProbaScenarios(s)*pPlanningReserveVoLL)
                         + sum((q,d,t,s), vUnmetSpinningReserveCountry(c,q,d,t,y,s)*pProbaScenarios(s)*pHours(q,d,t)*pSpinningReserveVoLL);
                         
eYearlyUnmetReserveCostSystem(y)..
   vYearlyUnmetReserveCostSystem(y) =e= vUnmetPlanningReserveSystem(y)*pPlanningReserveVoLL
                          + sum((q,d,t,s), vUnmetSpinningReserveSystem(q,d,t,y,s)*pProbaScenarios(s)*pHours(q,d,t)*pSpinningReserveVoLL);
...
*
   pDemandData(z,q,d,y,t)*pEnergyEfficiencyFactor(z,y) =e=sum((gzmap(g,z),gfmap(g,f)),vPwrOut(g,f,q,d,t,y,s))$(pIncludeH2=0)
*                                                            The row below is deactivated when H2 feature is on   
                                                        
***********************************************************H2 model equations below************************************************
                                                        +sum((gzmap(nRE,z),gfmap(nRE,f)),vPwrOut(nRE,f,q,d,t,y,s))$(pIncludeH2)
...
*                                                        + sum(gzmap(st,z), vStorOut(st,q,d,t,y))
                                                         - sum(gzmap(st,z), vStorInj(st,q,d,t,y,s))$(pincludeStorage)
                                                         + sum(zext, vImportPrice(z,zext,q,d,t,y,s)) 
                                                         - sum(zext, vExportPrice(z,zext,q,d,t,y,s)) 
                                                         + vUSE(z,q,d,t,y,s)
...
eJointResCap(g,q,d,t,y,s)..
   sum(gfmap(g,f), vPwrOut(g,f,q,d,t,y,s)) + vSpinningReserve(g,q,d,t,y,s) =l= vCap(g,y)*(1+pGenData(g,"Overloadfactor"));
...
eMinGen(g,q,d,t,y,s)$(Relevant(d) and pmingen_constraints and pGenData(g,"MinLimitShare") > 0)..
...
*--- Reserve equations
eSpinningReserveLim(g,q,d,t,y,s)$(pzonal_spinning_reserve_constraints or psystem_spinning_reserve_constraints)..
   vSpinningReserve(g,q,d,t,y,s) =l= vCap(g,y)*pGenData(g,"ResLimShare");
...
eSpinningReserveReqCountry(c,q,d,t,y,s)$pzonal_spinning_reserve_constraints..
   sum((zcmap(z,c),gzmap(g,z)),vSpinningReserve(g,q,d,t,y,s))
 + vUnmetSpinningReserveCountry(c,q,d,t,y,s)
 + sum((zcmap(z,c),sMapNCZ(z2,z)), pTransferLimit(z2,z,q,y)
   ...
   =g= pSpinningReserveReqCountry(c,y) + sum((zcmap(z,c),gzmap(VRE_noROR,z),gfmap(VRE_noROR,f)), vPwrOut(VRE_noROR,f,q,d,t,y,s))*pVREForecastError;
   

eSpinningReserveReqSystem(q,d,t,y,s)$psystem_spinning_reserve_constraints..
   sum(g, vSpinningReserve(g,q,d,t,y,s)) + vUnmetSpinningReserveSystem(q,d,t,y,s) =g= pSpinningReserveReqSystem(y) + sum(gfmap(VRE_noROR,f), vPwrOut(VRE_noROR,f,q,d,t,y,s))*pVREForecastError;


ePlanningReserveReqCountry(c,y,s)$(pplanning_reserve_constraints and pPlanningReserveMargin(c))..
   sum((zcmap(z,c),gzmap(g,z)), vCap(g,y)*pCapacityCredit(g,y))
 + vUnmetPlanningReserveCountry(c,y,s)
...
eMaxImportPrice(c,y,s)$(pallowExports)..
   sum((zcmap(z,c),zext,q,d,t), vImportPrice(z,zext,q,d,t,y,s)*pHours(q,d,t)) =l=
   sum((zcmap(z,c),q,d,t), pDemandData(z,q,d,y,t)*pHours(q,d,t)*pEnergyEfficiencyFactor(z,y))*pMaxExchangeShare(y,c);

eMaxExportPrice(c,y,s)$(pallowExports)..
   sum((zcmap(z,c),zext,q,d,t), vExportPrice(z,zext,q,d,t,y,s)*pHours(q,d,t)) =l=
   sum((zcmap(z,c),q,d,t), pDemandData(z,q,d,y,t)*pHours(q,d,t)*pEnergyEfficiencyFactor(z,y))*pMaxExchangeShare(y,c);


eMaxhourlyImportsshare(c,q,d,t,y,s)$(pMaxImport<1 and pallowExports)..

   sum((zcmap(z,c), zext), vImportPrice(z,zext,q,d,t,y,s))  =l= sum(zcmap(z,c), pDemandData(z,q,d,y,t)*pMaxImport * pEnergyEfficiencyFactor(z,y));

eMaxhourlyExportsshare(c,q,d,t,y,s)$(pMaxExport<1 and pallowExports)..
   sum((zcmap(z,c), zext),vExportPrice(z,zext,q,d,t,y,s)) =l= sum(zcmap(z,c), pDemandData(z,q,d,y,t)*pMaxExport * pEnergyEfficiencyFactor(z,y));


eExtZoneLimitImport(z,zext,q,d,t,y,s)$pallowExports..
   vImportPrice(z,zext,q,d,t,y,s)=l= pExtTransferLimitIn(z,zext,q,y);

eExtZoneLimitExport(z,zext,q,d,t,y,s)$pallowExports..
   vExportPrice(z,zext,q,d,t,y,s)=l= pExtTransferLimitOut(z,zext,q,y);
...
eRampDownInjLimit(st,q,d,t,y,s)$(not sFirstHour(t) and pincludeStorage and pramp_constraints)..
   vStorInj(st,q,d,t-1,y,s) - vStorInj(st,q,d,t,y,s) =l= pGenData(st,'RampDnRate')*vCap(st,y);

eRampUpInjLimit(st,q,d,t,y,s)$(not sFirstHour(t) and pincludeStorage and pramp_constraints)..
...
eStorBal(st,q,d,t,y,s)$(not sFirstHour(t) and pincludeStorage)..
   vStorage(st,q,d,t,y,s) =e= pStorData(st,"efficiency")*vStorInj(st,q,d,t,y,s) - sum(gfmap(st,f), vPwrOut(st,f,q,d,t,y,s)) + vStorage(st,q,d,t-1,y,s);

eStorBal1(st,q,d,sFirstHour(t),y,s)$pincludeStorage..
   vStorage(st,q,d,t,y,s) =e= pStorData(st,"efficiency")*vStorInj(st,q,d,t,y,s) - sum(gfmap(st,f), vPwrOut(st,f,q,d,t,y,s));
...
eStorageOutput(st,q,d,t,y,s)$pincludeStorage..
   vSpinningReserve(st,q,d,t,y,s) =l= vStorage(st,q,d,t,y,s);
...
eEmissionsCountry(c,y,s)$pzonal_co2_constraints..
   sum(zcmap(z,c), vZonalEmissions(z,y,s))-vYearlyCO2backstop(c,y,s)=l= pEmissionsCountry(c,y);
...
eFuelLimitH2(c,f,y,s)$(pFuelData(f) and pIncludeH2)..
   sum((zcmap(z,c)),  vFuel(z,f,y,s)) =e=  sum((zcmap(z,c)), vFuelH2(z,y));


*When the H2 production flag is off don't account for H2 fuel
eFuelLimitH2_2(c,f,y,s)$(pFuelData(f) and pIncludeH2=0)..
   sum((zcmap(z,c)),  vFuel(z,f,y,s)) =e=  0;
...
eRE2H2(RE,f,q,d,t,y,s)$pIncludeH2..
 vPwrOut(RE,f,q,d,t,y,s)=e=vREPwr2Grid(RE,f,q,d,t,y)+vREPwr2H2(RE,f,q,d,t,y);
