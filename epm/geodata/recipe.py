...
The map layers of EPM View are *derived* artefacts:

    zones_{stem}.geojson        one polygon per EPM zone
    linestring_{stem}.geojson   centroid-to-centroid lines between zones

A run writes its own pair into its results folder, cut from the zoning it
solved. The pair committed under ``epm/input/data_*/`` is what EPM View falls
back to for runs published before EPM did that, and for models that can no
longer be re-run.

Either way they are built by ``zone_layers.py`` from up to four sources:
...
The committed fallback pair is regenerated by hand, so editing any source
silently leaves it showing the previous zoning. To make that detectable, every
file written carries an ``epm_source`` member recording the SHA-256 of each
source it was built from. ``check_folder`` recomputes those hashes and reports
the files that no longer match.
...
GENERATOR = 'epm/geodata/zone_layers.py'
