...
* Storage data
   pStorDataInput(g,*,pStorageDataHeader)                 'Storage unit specifications'
...
* VRE and availability
   ...
   pAvailability(g,q)                                  'Seasonal availability factors (expanded to years)'
...
$load pStorDataInput pCSPData pCapexTrajectories pSpinningReserveReqCountry pSpinningReserveReqSystem 
...
pStorDataInput(g,g2,pStorageDataHeader) = pStorDataExcel(g,g2,pStorageDataHeader);
pStorDataInput(g,g,pStorageDataHeader)$pStorDataExcel(g,'',pStorageDataHeader) = pStorDataExcel(g,'',pStorageDataHeader);
...
loop(gsmap(g2,g), pStorData(g,pStorageDataHeader) = pStorDataInput(g,g2,pStorageDataHeader));
