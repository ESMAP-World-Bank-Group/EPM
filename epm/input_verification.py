...
def _check_storage_data(gams, db):
    """Verify that storage generators have matching entries in pStorDataInput."""
    try:
        stor_records = db["pStorDataInput"].records
        ...
        if missing_storage_gen:
            msg = (
                "Error: The following storage genartors are in pGenDataInput but not defined in pStorDataInput: \n"
                f"{missing_storage_gen}"
            )
            gams.printLog(msg)
            raise ValueError(msg)
        gams.printLog('Success: All storage generators are are well-defined in pStorDataInput.')
    except ValueError:
        raise
    except Exception:
