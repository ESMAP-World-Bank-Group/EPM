...
* Fuels
   ...
   pMaxFuellimitDay(c,f,y)             'Fuel limit in MMBTU*1e6 (million) by country per day'
...
eFuelLimitDayLNG(c,q,d,y)              'fuel limit at country level'
 ...
 eMaxAnnualImportShareEnergy(c,y)
 eMaxAnnualExportShareEnergy(c,y)
eStorageCap(g,q,d,t,y)
 eStorageCap1(g,q,d,t,y) 
   eStorageCap2(g,q,d,t,y)         'storage capacity (energy) must be at least 1 hour if installed'
   eStorageCap3(g,q,d,t,y) 
   eStorageCap4(g,q,d,t,y)         'storage capacity (energy) must be at least 1 hour if installed'
   eStorageCap5(g,q,d,t,y) 
   eStorageCap6(g,q,d,t,y)         'storage capacity (energy) must be at least 1 hour if installed'
   eMinREAnnual(y)
   eWindLimit(y)
...
eStorageCap1(st,q,d,t,y)$(pincludeStorage and pGenData(st,'fuel1')=22)..
   vCapStor(st,y) =e= vCap(st,y);

eStorageCap2(st,q,d,t,y)$(pincludeStorage and pGenData(st,'fuel1')=23)..
   vCapStor(st,y) =e= 2*vCap(st,y);
   

eStorageCap3(st,q,d,t,y)$(pincludeStorage and pGenData(st,'fuel1')=24)..
   vCapStor(st,y) =e= 3*vCap(st,y);
   
eStorageCap4(st,q,d,t,y)$(pincludeStorage and pGenData(st,'fuel1')=25)..
   vCapStor(st,y) =e= 4*vCap(st,y);

eStorageCap5(st,q,d,t,y)$(pincludeStorage and pGenData(st,'fuel1')=26)..
   vCapStor(st,y) =e= 5*vCap(st,y);

eStorageCap6(st,q,d,t,y)$(pincludeStorage and pGenData(st,'fuel1')=27)..
   vCapStor(st,y) =e= 6*vCap(st,y);
...
*eStorBal(st,q,d,t,y)$(ord(t)>1 and pincludeStorage  )..
...
* vStorage(st,q,d,t,y) =e= pStorData(st,"efficiency")*vStorInj(st,q,d,t,y) - Sum(gfmap(st,f),vPwrOut(st,f,q,d,t,y))+ 0.5*vCapStor(st,y);
...
eStorBal1(st,q,d,sFirstHour(t),y)$(pincludeStorage  )..
  vStorage(st,q,d,t,y) =e= pStorData(st,"efficiency")*vStorInj(st,q,d,t,y) - Sum(gfmap(st,f),vPwrOut(st,f,q,d,t,y))+ 0.1*vCapStor(st,y);
...
eMinREAnnual(y)..      sum((zcmap(z,c),gzmap(RE,z),gfmap(RE,f),q,d,t), vPwrOut(RE,f,q,d,t,y)*pHours(q,d,y,t))     =g=    sum((zcmap(z,c),gzmap(g,z),gfmap(g,f),q,d,t), vPwrOut(g,f,q,d,t,y)*pHours(q,d,y,t))*pMinREAnnual(y);


eFuelLimitGAS(c,y)$(pfuel_constraints and sum(GASALL,pMaxFuelLimit(c,GASALL,y)) > 0)..
   sum((zcmap(z,c),zfmap(z,GASALL)), vFuel(z,GASALL,y)) =l= sum(GASALL, pMaxFuelLimit(c,GASALL,y))*1e6;
   
eFuelLimitLNG(c,y)$(pfuel_constraints and sum(LNGALL,pMaxFuelLimit(c,LNGALL,y)) > 0)..
   sum((zcmap(z,c),zfmap(z,LNGALL)), vFuel(z,LNGALL,y)) =l= sum(LNGALL,pMaxFuelLimit(c,LNGALL,y))*1e6;


eFuelLimitDayLNG(c,q,d,y)$(pfuel_constraints and sum(LNGALL,pMaxFuelLimitDay(c,LNGAll,y)) > 0)..
 sum((zcmap(z,c),gzmap(g,z),gfmap(g,LNGAll),t), vPwrOut(g,LNGAll,q,d,t,y)*pHours(q,d,y,t)*pHeatRate(g,LNGAll)) =l= sum(LNGALL,pMaxFuelLimitDay(c,LNGALL,y))*1e6*pNumDays(q,d,y);
 
eFuelLimitDayGas(c,q,d,y)$(pfuel_constraints and sum(GASALL,pMaxFuelLimitDay(c,GasAll,y)) > 0)..
 sum((zcmap(z,c),gzmap(g,z),gfmap(g,GasAll),t), vPwrOut(g,GasAll,q,d,t,y)*pHours(q,d,y,t)*pHeatRate(g,GasAll)) =l= sum(GASALL,pMaxFuelLimitDay(c,GASALL,y))*1e6*pNumDays(q,d,y);
...
eMaxAnnualImportShareEnergy(c,y)..
   sum((zcmap(z,c),zext,q,d,t), vImportPrice(z,zext,q,d,t,y)*pHours(q,d,y,t)) =l=
   sum((zcmap(z,c),q,d,t), pDemandData(z,q,d,y,t)*pHours(q,d,y,t))*0.1;
