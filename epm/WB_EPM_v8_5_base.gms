...
**The code is used for the SAPP model
**Added/modified functions: 

*   eMinUtilization(g,y)           'Minimum annual utilization for generators'
*   eStorageCap2(st,q,d,t,y)
*   eStorBal3(g,q,d,t,y)
...
*   eInjCSP2(cs,q,d,t,y)
*   eInjCSP3(cs,q,d,t,y)
*  new parameters: MinUtilizationRate(g,y)
*-------------------------------------------------------------------
...
Sets
   ghdr         'Header for pGenData' / BuildLimitperYear, Capacity, Capex, DescreteCap, FOMperMW, MaxTotalBuild,
                                        MinLimitShare, MinTotalBuild, Overloadfactor, RampDnRate, RampUpRate, ReserveCost, ResLimShare, RetrYr, StYr, UnitSize, MinUtilization /
...
*   eSpinningReserveLimVRE(g,q,d,t,y)           'Reserve limit for VRE as a share of capacity adjusted for production profile'
   ...
   eTransferLimitMin(z2,z,q,d,t,y) 'Minimum import constraint'
   eAnnualTransferLimit(z,z2,y)    'Annual transfer limits /Mozambique to South Africa'
...
*   eEmissionsTotal(y)              'constraint on total regional CO2eq emissions by year in tons'
   eYearlyCO2backstopCost(c,y)     'co2 backstop cost in USD'                                                                                     
   eTotalEmissionsConstraint(y)
   eTotalEmissionsConstraintSum    'Emission budget for the whole system and whole period'
...
eCapBalance(g,sStartYear(y))..
   vCap(g,y) =e= pGenData(g,"Capacity")$(eg(g) and (pGenData(g,"StYr") <= sStartYear.val) and (pGenData(g,"RetrYr") >= sStartYear.val))
...
eMaxCF(g,q,y)..
   sum((gfmap(g,f),d,t), vPwrOut(g,f,q,d,t,y)*pHours(q,d,y,t)) =l= pHydroVar(g,y)*pAvailability(g,q)*vCap(g,y)*sum((d,t), pHours(q,d,y,t));
...
eMinUtilization(g,y)$(pMinUtilizationRate(g,y)>0)..
   Sum((gfmap(g,f),q,d,t),vPwrOut(g,f,q,d,t,y)*pHours(q,d,y,t)) =g= vCap(g,y)*Sum((q,d,t),pHours(q,d,y,t))*pMinUtilizationRate(g,y) ;
...
ePlanningReserveReqCountry(c,y)$(pplanning_reserve_constraints and pPlanningReserveMargin(c,y) and y.val >= pzonal_planning_Startyr)..
 ...
 + sum((zcmap(z,c),sMapNCZ(z2,z)), sum(q,pTransferLimit(z2,z,q,y))/card(q) + vAdditionalTransfer(z2,z,y)*symmax(pNewTransmission,z,z2,"CapacityPerLine"))
   =g= (1+pPlanningReserveMargin(c,y))*smax((q,d,t), sum(zcmap(z,c), pDemandData(z,q,d,y,t)*pEnergyEfficiencyFactor(z,y)));
...
eMaxImportregional(c,y)..
   Sum((zcmap(z,c),q,d,t),(-Sum(sTopology(z,z2),vFlow(z,z2,q,d,t,y)) + Sum(sTopology(z,z2),vFlow(z2,z,q,d,t,y)*(1-pLossFactor(z,z2,y)))) * pHours(q,d,y,t)) =l=
   Sum((zcmap(z,c),q,d,t),pDemandData(z,q,d,y,t)*pHours(q,d,y,t)* pEnergyEfficiencyFactor(z,y))*pMaxPriceImportShare(y,c);
...
*Exports from Cohora Bassa (Mozambique) to South Africa
eAnnualTransferLimit(sTopology(z,z2),y)$(pZoneIndex(z)=4 and pZoneIndex(z2)=1 and y.val<2030)..
*   sum((q,d,t),vFlow(z,z2,q,d,t,y)*pHours(q,d,y,t)) - sum((q,d,t),vFlow(z2,z,q,d,t,y)*pHours(q,d,y,t)) =g= 1375*8760*0.8;
    sum((q,d,t),vFlow(z,z2,q,d,t,y)*pHours(q,d,y,t)) =g= 1375*8760*0.8;

    
*Allow 10 TWh free market trade: South Africa to Moz
*eAnnualTransferLimit2(sTopology(z,z2),y)$(pZoneIndex(z)=1 and pZoneIndex(z2)=4 and y.val<2026)..
*   sum((q,d,t),vFlow(z,z2,q,d,t,y)*pHours(q,d,y,t)) =l= 1e5 ;
...
eStorageCap2(st,q,d,t,y)$pincludeStorage..
   vCapStor(st,y) =g= pMinStorageHrs * vCap(st,y);
...
eStorBal2(st,q,d,sFirstHour(t),y)$(not sFirstDay(d) and pincludeStorage)..
   vStorage(st,q,d,t,y) =e= pStorData(st,"efficiency")*vStorInj(st,q,d,t,y)
                          - sum(gfmap(st,f), vPwrOut(st,f,q,d,t,y)) + vStorage(st,q,d-1,sLastHour,y);
...
*new
eInjCSP2(cs,q,d,t,y)$(pincludeCSP=1)..
    vCapTherm(cs,y)  =g= vCap(cs,y);
*new
eInjCSP3(cs,q,d,t,y)$(pincludeCSP=1)..
    vCapStor(cs,y)  =g= (pMinStorageHrs-1) * vCap(cs,y);
*eInjCSP3(cs,q,d,t,y)$(pincludeCSP=1)..
**  vCapStor(cs,y)  =g= vCap(cs,y);
...
*New: CSP profile
eCSPProfile(cs,q,d,t,y)$(pincludeCSP=1)..
   Sum(gfmap(cs,f),vPwrOut(cs,f,q,d,t,y)) + vSpinningReserve(cs,q,d,t,y) =l= vCap(cs,y);
...
eBuildStorNew(eg)$((pGenData(eg,"StYr") > sStartYear.val) and pincludeStorage)..
   sum(y, vBuildStor(eg,y)) =l= pStorData(eg,"Capacity") + pCSPData(eg,"Storage","Capacity") ;
...
*New function for emission budget   
