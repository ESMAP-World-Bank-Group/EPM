...
Option limRow=10000, limCol=0, sysOut=off, solPrint=off;
...
$onmulti
Sets
   ghdr         'Additional headers for pGenData' / CapacityCredit, Heatrate, Heatrate2, Life, VOM, fuel1,
*************************Dispatch model******************************************
...
**************************************Dispatch*********************88
set=mapTS                   rdim=4  rng=DispatchSetMapping!A3:W99999
...
****************************Dispatch****************************************************
pRunDispatch                        = pScalars("RunDispatch");
pIncludeMUDT                        = pScalars("IncludeMUDT");
pIncludeMinGenCon                   = pScalars("IncludeMinGenCon");
pIncludeStUp                        = pScalars("IncludeStUp");
pIncludeDR                          = pScalars("IncludeDR");
pCostOfDR                           = pScalars("CostOfDR");
pMaxDR                              = pScalars("MaxDR");
...
**********************************************Dispatch**********************************************************
$IF NOT "%RunDispatch%"                         == "-1" pRunDispatch                         = %RunDispatch%;
$IF NOT "%IncludeMUDT%"                         == "-1" pIncludeMUDT                         = %IncludeMUDT%;
$IF NOT "%IncludeMinGenCon%"                    == "-1" pIncludeMinGenCon                    = %IncludeMinGenCon%;
$IF NOT "%IncludeStUp%"                         == "-1" pIncludeStUp                         = %IncludeStUp%;
$IF NOT "%IncludeDR%"                           == "-1" pIncludeDR                           = %IncludeDR%;
...
MinGenPoint(g)  = pGenData(g,"MinGenPoint");
MinUpTime(g)    = pGenData(g,"MinUT");
MinDnTime(g)    = pGenData(g,"MinDT");
HoursOn(g)      = pGenData(g,"HoursOn");
HoursOff(g)     = pGenData(g,"HoursOff");
FD(q,d,t)       =yes;
...
***************Dispatch*********************************
*************IAEA***(correct 29th for February on dispatch version)******************

parameter
   pDays(q)                  'Number of days for each month'  / m1=31
                                                                m2=28 
                                                                m3=31
                                                                m4=30
                                                                m5=31
...
vStorInj.fx(st,q,d,t,y)$(not FD(q,d,t))  =   0;
...
display pRR, pWeightYear, pGenData, pCapacityCredit, g, eg, ng,pEnergyEfficiencyFactor, pDemandData, pStorData, sFirstDay, sFirstHour,sFirstQuarter,pRunDispatch, pIncludeMUDT, pIncludeMinGenCon;
display pIncludeStUp, pIncludeDR, pCostOfDR, pMaxDR;
display ramprate,pramp_constraints;
display AT, mapTS, sFirstHourAT, mingenpoint, c, y, pRR, pWeightYear, FD;
