...
- [Armenia](#armenia) — [`pStorageDataInput`](#armenia-pstoragedatainput) · [`pDemandForecast`](#armenia-pdemandforecast) · [`pDemandProfile`](#armenia-pdemandprofile) · [`pVREProfile`](#armenia-pvreprofile) · [`pAvailabilityCustom`](#armenia-pavailabilitycustom) · [`pGenDataInput`](#armenia-pgendatainput) · [`pFuelPrice`](#armenia-pfuelprice) · [`pNewTransmission`](#armenia-pnewtransmission)
- [Georgia](#georgia) — [`pStorageDataInput`](#georgia-pstoragedatainput) · [`pGenDataInput`](#georgia-pgendatainput) · [`pDemandForecast`](#georgia-pdemandforecast) · [`pDemandProfile`](#georgia-pdemandprofile) · [`pVREProfile`](#georgia-pvreprofile) · [`pFuelPrice`](#georgia-pfuelprice) · [`pAvailabilityCustom`](#georgia-pavailabilitycustom) · [`pMaxFuelLimit`](#georgia-pmaxfuellimit) · [`pTransferLimit`](#georgia-ptransferlimit) · [`pLossFactorInternal`](#georgia-plossfactorinternal) · [`pExtTransmissionCost`](#georgia-pexttransmissioncost) · [`zcmap`](#georgia-zcmap)
- [Azerbaijan](#azerbaijan) — [`pVREProfile`](#azerbaijan-pvreprofile) · [`pGenDataInput`](#azerbaijan-pgendatainput) · [`pDemandForecast`](#azerbaijan-pdemandforecast) · [`pDemandProfile`](#azerbaijan-pdemandprofile) · [`pFuelPrice`](#azerbaijan-pfuelprice) · [`pAvailabilityCustom`](#azerbaijan-pavailabilitycustom) · [`pTransferLimit`](#azerbaijan-ptransferlimit) · [`pTradePrice`](#azerbaijan-ptradeprice) · [`pTradePriceExport`](#azerbaijan-ptradepriceexport) · [`pMaxAnnualExternalTradeShare`](#azerbaijan-pmaxannualexternaltradeshare) · [`pLossFactorInternal`](#azerbaijan-plossfactorinternal) · [`zcmap`](#azerbaijan-zcmap)
...
|---|---|---|
| [`pGenDataInput`](#turkiye-pgendatainput) | Observed annual capacity addit… (2026) + World Bank EPM Türkiye Least-Cost Model v7 (2025) + [Ramp rates and minimum generation shares (CCDR parameter set, reachable subset)](https://esmap-world-bank-group.github.io/EPM/input/input_parameter_guide/) + [MENR / TEİAŞ](https://www.teias.gov.tr/tr-TR/kurulu-guc-raporlari) + [TEİAŞ](https://www.teias.gov.tr/tr-TR/turkiye-elektrik-istatistikleri) | [MEDIUM] ⚠ |
...
**Also uses**: [MENR / TEİAŞ — Turkiye installed capacity as of May 2026 (official chart)](https://www.teias.gov.tr/tr-TR/kurulu-guc-raporlari)

**Also uses**: [TEİAŞ — Turkiye annual electricity statistics (official)](https://www.teias.gov.tr/tr-TR/turkiye-elektrik-istatistikleri)
...
| Period | Method | Notes |
|--------|--------|-------|
| starting fleet check, 2026-09-14 | `DIRECT` | Existing fleet (Status 1) checked against the official May 2026 installed capacity chart [menr_installed_capacity_2026_05] and the TEİAŞ end-2024 statistic. The fleet is the end-2024 TEİAŞ fleet under the 2025 label: gas 24,665 MW, coal 21,931, hydro 32,203, solar 20,153, wind 13,341, geothermal 1,734, total 116,796 MW, all within 100 MW of TEİAŞ end 2024 (116,265 MW). Against May 2026 (125,471 MW) every fuel matches except solar: 26,830 MW official (24,102 unlicensed) against 20,453 in the model for 2026, a 6.4 GW gap that neither the v7 fleet nor the committed PV rows (271 MW in 2025, 300 in 2026) capture. Gas is 0.8 GW above the statistic because the committed 890 MW CCGT is dated 2026. DECISION. Fleet kept unchanged for the September 2026 scenario grid: the gap only touches 2025-2026, the baseline builds 8,788 MW of PV in 2027 on its own (29,241 MW in 2027, above the May 2026 statistic), and the gap is the same in every scenario so it cancels in the corridor deltas. TO DO. Rebase the existing fleet of every country on end 2025 in one pass once the TEİAŞ 2025 annual statistics are published. |
...
| no unplanned retirement, 2026-09-05 | `ASSUMPTION` | Every retirement date inside the 2025-2040 horizon that is not a published plan date is pushed beyond the horizon. WHY. The 2026-08-29 cleanup rewrote only the rows carrying the mechanical StYr + 25/30/50 placeholder and deliberately left the rest, so the model was still closing plants on inherited age assumptions rather than on any announced schedule: the Georgian gas fleet lost 693 MW between 2027 and 2036, the Azerbaijani CHP bridge 985 MW in 2029, and 2,256 MW of Turkish thermal left the fleet by 2038. None of those dates is traceable to an operator or government decommissioning plan. A retirement the plan does not contain is a modelling assumption that silently creates a capacity need and biases every investment and trade result built on it. RULE APPLIED (RetrYr and Life columns only): a row is touched if it sits in a modelled zone (zcmap.csv), has Status 1 or 2, and RetrYr is 2040 or earlier. It receives RetrYr = 2060 and Life = 2060 - StYr. 2060 is the floor the 2026-08-29 pass already used for hydro; it is beyond the horizon, so with fEnableEconomicRetirement = 0 main.gms:832 pins vCap at nameplate for every horizon year and main.gms:814 never fires. Life is kept arithmetically coherent with RetrYr = StYr + Life so the two columns stay auditable; on these rows it is a bookkeeping value, NOT a technical life estimate, and it is inert for cost because Capex is blank on every row touched, so pCRF multiplies a zero capex. 15 rows across four countries. Storage (pStorageDataInput) has no row retiring inside the horizon and is untouched, and no Status 3 candidate retires inside the horizon. KEPT: Armenia_ANPP 407.5 MW retires 2036, a published Rosatom life-extension date, with Armenia_Nuclear_Cand standing as its successor candidate. WHAT THIS IS NOT. It is not a claim that these plants will run to 2060. It is the statement that the study does not close a plant the plan does not close, and that the horizon end, not an assumed age, is where the fleet is cut. The refurbishment those extensions imply is NOT costed: Capex is blank on all 15 rows, so the life extension is free to the optimiser. Two VRE rows are the weakest case, 25 yr being a real design life rather than an inherited placeholder: Nakhchivan_Solar_PV 20 MW and Azerbaijan_Khizi_Wind 14 MW, 34 MW in total, are kept online on the assumption of repowering in place at zero capex. Replace any of these dates on sight if a published schedule appears. THIS COUNTRY: 6 rows, 2,255.6 MW kept online. AFSIN_ELBISTAN_A1 340 MW, A2 340 MW, A3 340 MW and A4 335 MW (ST/DomesticCoal, 2038 to 2060); EUAS_ISTANBUL_A1_NG and A2_NG 450.3 MW each (CCGT/Gas, 2028 to 2060). All six dates are inherited from WB EPM Turkiye v7 and none is documented there as a published closure schedule. |

*Confidence: [MEDIUM] · Last updated: 2026-09-05*
...
| [`pDemandProfile`](#armenia-pdemandprofile) | proxy of Georgia | [LOW] ⚠ |
...
**Proxied from**: Georgia  
**Original source**: ENTSO-E Transparency Platform — Actual Total Load (hourly)

> ⚠ **Needs review**: Obtain EPSO hourly load data for Armenia to replace the Georgia proxy

**Method**: PROXY_Georgia
...
| Period | Method | Notes |
|--------|--------|-------|
| 2037 to 2053, LC_ArmNuke and its CTN and ArTur variants | `CHANGED` | CHANGED 2026-09-15. Forced successor to the Metsamor plant, read by LC_ArmNuke, LC_CTN_ArmNuke and LC_ArTur_ArmNuke (the last two force the CTN or ArTur line on top) through supply/pGenDataInput_armnuke.csv. That file is supply/pGenDataInput.csv with one row changed: Armenia_Nuclear_Cand goes from Status 3 (candidate) to Status 2 (committed), StYr 2036 to 2037 and RetrYr 2086 to 2087 (Life 50 kept). 2037 is the first model year without Armenia_ANPP (407.5 MW, RetrYr 2036), so the two plants never run in the same year. A committed plant is built for its full Capacity, 300 MW, at StYr (main.gms:728) and pays the candidate Capex, 8 $m/MW flat (the Armenia Nuclear capex trajectory is all zero, so the plant sits outside dc). LC_ArmNuke is LC_Baseline otherwise, the Georgia to Armenia line included. In every other scenario the candidate stays optional at StYr 2036 and is never built. |
...
| no unplanned retirement, 2026-09-05 | `ASSUMPTION` | Every retirement date inside the 2025-2040 horizon that is not a published plan date is pushed beyond the horizon. WHY. The 2026-08-29 cleanup rewrote only the rows carrying the mechanical StYr + 25/30/50 placeholder and deliberately left the rest, so the model was still closing plants on inherited age assumptions rather than on any announced schedule: the Georgian gas fleet lost 693 MW between 2027 and 2036, the Azerbaijani CHP bridge 985 MW in 2029, and 2,256 MW of Turkish thermal left the fleet by 2038. None of those dates is traceable to an operator or government decommissioning plan. A retirement the plan does not contain is a modelling assumption that silently creates a capacity need and biases every investment and trade result built on it. RULE APPLIED (RetrYr and Life columns only): a row is touched if it sits in a modelled zone (zcmap.csv), has Status 1 or 2, and RetrYr is 2040 or earlier. It receives RetrYr = 2060 and Life = 2060 - StYr. 2060 is the floor the 2026-08-29 pass already used for hydro; it is beyond the horizon, so with fEnableEconomicRetirement = 0 main.gms:832 pins vCap at nameplate for every horizon year and main.gms:814 never fires. Life is kept arithmetically coherent with RetrYr = StYr + Life so the two columns stay auditable; on these rows it is a bookkeeping value, NOT a technical life estimate, and it is inert for cost because Capex is blank on every row touched, so pCRF multiplies a zero capex. 15 rows across four countries. Storage (pStorageDataInput) has no row retiring inside the horizon and is untouched, and no Status 3 candidate retires inside the horizon. KEPT: Armenia_ANPP 407.5 MW retires 2036, a published Rosatom life-extension date, with Armenia_Nuclear_Cand standing as its successor candidate. WHAT THIS IS NOT. It is not a claim that these plants will run to 2060. It is the statement that the study does not close a plant the plan does not close, and that the horizon end, not an assumed age, is where the fleet is cut. The refurbishment those extensions imply is NOT costed: Capex is blank on all 15 rows, so the life extension is free to the optimiser. Two VRE rows are the weakest case, 25 yr being a real design life rather than an inherited placeholder: Nakhchivan_Solar_PV 20 MW and Azerbaijan_Khizi_Wind 14 MW, 34 MW in total, are kept online on the assumption of repowering in place at zero capex. Replace any of these dates on sight if a published schedule appears. THIS COUNTRY: 1 row, 0 MW. Armenia_Hrazdan_ST 2030 to 2060, applied for consistency with the rule; the row is inert because its Capacity is 0, units 1-4 being mothballed per GEM 2024. Armenia_ANPP keeps 2036, see KEPT above. |
...
*Confidence: [MEDIUM] · Last updated: 2026-09-15*
...
<a id="armenia-pnewtransmission"></a>
...
> ⚠ **Needs review**: Replace capacity, cost and entry year with a project estimate if the Armenia-Turkiye interconnection is documented; the political precondition (border reopening) is not modelled.


**Method**: ASSUMPTION - Armenia to EastAna (Turkiye) candidate added 2026-09-14 for the LC_ArTur scenario

| Period | Method | Notes |
|--------|--------|-------|
| 2030 to 2053 | `ASSUMPTION` | ADDED 2026-09-14, written directly in the trade/pNewTransmission_*.csv files (the reference_lines.csv pipeline was not rerun). The line database only holds the idle Kars-Gyumri 154/220 kV link (0 MW, closed border, 300 MW maximum capacity per the TEIAS EK-1 sheet). The candidate is therefore 1 line of 300 MW, earliest entry 2030, life 30 years, 730 MUSD per line: the BSTN back-to-back unit cost (850 MUSD for 350 MW) scaled to 300 MW, no project-specific estimate exists. Forced (status 2) in pNewTransmission_artur.csv and _allprojects.csv, free with up to 4 lines in the two free-expansion files, 0 lines elsewhere so every file carries the same candidate set. Loss factor: the 0.025 Armenia-EastAna rows already in pLossFactorInternal.csv. |

*Confidence: [LOW] · Last updated: 2026-09-14*
...
| [`pDemandProfile`](#georgia-pdemandprofile) | ENTSO-E Transparency Platform (2025) + [Black Sea hourly load + VRE, representative-days pipeline (ENTSO-E · EPİAŞ · Renewables Ninja)](https://transparency.entsoe.eu/) | [MEDIUM] |
...
| no unplanned retirement, 2026-09-05 | `ASSUMPTION` | Every retirement date inside the 2025-2040 horizon that is not a published plan date is pushed beyond the horizon. WHY. The 2026-08-29 cleanup rewrote only the rows carrying the mechanical StYr + 25/30/50 placeholder and deliberately left the rest, so the model was still closing plants on inherited age assumptions rather than on any announced schedule: the Georgian gas fleet lost 693 MW between 2027 and 2036, the Azerbaijani CHP bridge 985 MW in 2029, and 2,256 MW of Turkish thermal left the fleet by 2038. None of those dates is traceable to an operator or government decommissioning plan. A retirement the plan does not contain is a modelling assumption that silently creates a capacity need and biases every investment and trade result built on it. RULE APPLIED (RetrYr and Life columns only): a row is touched if it sits in a modelled zone (zcmap.csv), has Status 1 or 2, and RetrYr is 2040 or earlier. It receives RetrYr = 2060 and Life = 2060 - StYr. 2060 is the floor the 2026-08-29 pass already used for hydro; it is beyond the horizon, so with fEnableEconomicRetirement = 0 main.gms:832 pins vCap at nameplate for every horizon year and main.gms:814 never fires. Life is kept arithmetically coherent with RetrYr = StYr + Life so the two columns stay auditable; on these rows it is a bookkeeping value, NOT a technical life estimate, and it is inert for cost because Capex is blank on every row touched, so pCRF multiplies a zero capex. 15 rows across four countries. Storage (pStorageDataInput) has no row retiring inside the horizon and is untouched, and no Status 3 candidate retires inside the horizon. KEPT: Armenia_ANPP 407.5 MW retires 2036, a published Rosatom life-extension date, with Armenia_Nuclear_Cand standing as its successor candidate. WHAT THIS IS NOT. It is not a claim that these plants will run to 2060. It is the statement that the study does not close a plant the plan does not close, and that the horizon end, not an assumed age, is where the fleet is cut. The refurbishment those extensions imply is NOT costed: Capex is blank on all 15 rows, so the life extension is free to the optimiser. Two VRE rows are the weakest case, 25 yr being a real design life rather than an inherited placeholder: Nakhchivan_Solar_PV 20 MW and Azerbaijan_Khizi_Wind 14 MW, 34 MW in total, are kept online on the assumption of repowering in place at zero capex. Replace any of these dates on sight if a published schedule appears. THIS COUNTRY: 4 rows, 693.2 MW kept online. Georgia_Tbilsresi_CCGT 270 MW (2027 to 2060), Georgia_Mtkvari_CCGT 300 MW (2030 to 2060), Georgia_GPower_OCGT 110 MW (2031 to 2060), Georgia_Tkibuli_ST 13.2 MW (2036 to 2060). This is the fleet whose disappearance drove the Georgian gas contraction in the 2026-09-02 results; those results are superseded. The Tbilsresi 2027 date was flagged in this file as estimated, to be confirmed with CESI or GSE, and never was; it now sits outside the horizon, so the question no longer changes the answer inside 2025-2040. |
...
**Method**: DIRECT 2023 hourly load from the ENTSO-E Transparency Platform, reduced to 28 representative days
...
| 2024 to 2053, LC_Baseline_NoArGe and its CTN and ArTur variants | `CHANGED` | CHANGED 2026-09-15. Sensitivity without the existing Georgia to Armenia line, read by LC_Baseline_NoArGe, LC_CTN_NoArGe and LC_ArTur_NoArGe (the last two force the CTN or ArTur line on top) through trade/pTransferLimit_noarge.csv. That file is trade/pTransferLimit.csv with the eight Armenia to Georgia and Georgia to Armenia rows (Q1 to Q4, both directions) set from 150 MW to 0 MW in every year; every other row is identical. The pair stays in sTopology and the Georgia to Armenia row of pNewTransmission_none.csv keeps MaximumNumOfLines 0, so nothing can be built on it. LC_Baseline and every other scenario keep the line at 150 MW both ways. |

*Confidence: [MEDIUM] · Last updated: 2026-09-15*


<a id="georgia-plossfactorinternal"></a>
...
> ⚠ **Needs review**: The losses of the existing internal lines have no source in the catalog. Document them or replace them with a sourced value.


**Method**: CESI bus-bar to bus-bar losses for the GEC layer. The existing internal lines are unchanged

| Period | Method | Notes |
|--------|--------|-------|
| 2024 to 2053 | `ASSUMPTION` | Georgia to Armenia, AzerbaijanMain and EastAna (Turkiye): model values kept unchanged. No catalog source documents them. |
| 2032 to 2053, GECO scenarios | `CHANGED` | CHANGED 2026-09-11. GEC layer, read only by the five GECO scenarios through zcmap_geco.csv. GEC_AZ to GEC_GE leg at the CESI bus-bar to bus-bar loss, converter stations included. GEC_GE to Georgia tap at a technical floor, because EPM rejects a zero loss. Both directions. The leg loss is weighted by the GEC links in service (T5 p79, p81, p83). Values in cesi/cesi_register.yaml (k04). Source CESI GEC T5 p76, p79, p81 and p83. The other scenarios drop these rows through the zone filter. |

*Confidence: [MEDIUM] · Last updated: 2026-09-11*


<a id="georgia-pexttransmissioncost"></a>

### `pExtTransmissionCost`
...
> ⚠ **Needs review**: The BSSC phase cost is an assumption. Replace it with a phase level estimate.


**Method**: Corridor capex catalogue in trade/pExtTransmissionCost.csv, read ex post by tools/ext_transmission_cost.py. GAMS never reads it

| Period | Method | Notes |
|--------|--------|-------|
| BSSC phase | `ASSUMPTION` | Georgia to Romania BSSC phase: pro rata share of the full corridor cost. Not in the v8 EPM Entry sheet. Unchanged. |
| GEC links, GECO scenarios | `CHANGED` | CHANGED 2026-09-11. GEC_GE to Romania, one row per GEC link. Capex of the AZ-GE and GE-RO sections, converted to USD. The RO-HU section is excluded because Hungary is not modelled. Capacity derated for availability as in the GECO link files. Asset life kept as for the other corridors and opex not carried, both decided in P4. The former GECO row, which carried the full corridor on Georgia to Romania, is removed. Values in cesi/cesi_register.yaml (k09). Source CESI GEC T5 p79, p81 and p83. |

*Confidence: [MEDIUM] · Last updated: 2026-09-11*


<a id="georgia-zcmap"></a>
...
**Method**: Zone to country map. zcmap.csv serves every scenario, zcmap_geco.csv adds the GEC zones for the five GECO scenarios
...
| [`pDemandProfile`](#azerbaijan-pdemandprofile) | proxy of Turkiye (EPIAS national hourly shape) | [LOW] ⚠ |
...
| [`pTransferLimit`](#azerbaijan-ptransferlimit) | Black Sea Cross-Border Lines D… (2026) + Modeller expert judgment + CESI, Green Energy Corridor (Azerbaijan, Georgia, Romania, Hungary) feasibility study, Tasks 2 to 8 🔒 | [MEDIUM] ⚠ |
| [`pLossFactorInternal`](#azerbaijan-plossfactorinternal) | CESI, Green Energy Corridor (A… (2026-08) 🔒 | [MEDIUM] ⚠ |
| [`pTradePrice`](#azerbaijan-ptradeprice) | Kazakh border price for the Tr… (2026) + Modeller expert judgment | [MEDIUM] |

<a id="azerbaijan-pvreprofile"></a>
...
| no unplanned retirement, 2026-09-05 | `ASSUMPTION` | Every retirement date inside the 2025-2040 horizon that is not a published plan date is pushed beyond the horizon. WHY. The 2026-08-29 cleanup rewrote only the rows carrying the mechanical StYr + 25/30/50 placeholder and deliberately left the rest, so the model was still closing plants on inherited age assumptions rather than on any announced schedule: the Georgian gas fleet lost 693 MW between 2027 and 2036, the Azerbaijani CHP bridge 985 MW in 2029, and 2,256 MW of Turkish thermal left the fleet by 2038. None of those dates is traceable to an operator or government decommissioning plan. A retirement the plan does not contain is a modelling assumption that silently creates a capacity need and biases every investment and trade result built on it. RULE APPLIED (RetrYr and Life columns only): a row is touched if it sits in a modelled zone (zcmap.csv), has Status 1 or 2, and RetrYr is 2040 or earlier. It receives RetrYr = 2060 and Life = 2060 - StYr. 2060 is the floor the 2026-08-29 pass already used for hydro; it is beyond the horizon, so with fEnableEconomicRetirement = 0 main.gms:832 pins vCap at nameplate for every horizon year and main.gms:814 never fires. Life is kept arithmetically coherent with RetrYr = StYr + Life so the two columns stay auditable; on these rows it is a bookkeeping value, NOT a technical life estimate, and it is inert for cost because Capex is blank on every row touched, so pCRF multiplies a zero capex. 15 rows across four countries. Storage (pStorageDataInput) has no row retiring inside the horizon and is untouched, and no Status 3 candidate retires inside the horizon. KEPT: Armenia_ANPP 407.5 MW retires 2036, a published Rosatom life-extension date, with Armenia_Nuclear_Cand standing as its successor candidate. WHAT THIS IS NOT. It is not a claim that these plants will run to 2060. It is the statement that the study does not close a plant the plan does not close, and that the horizon end, not an assumed age, is where the fleet is cut. The refurbishment those extensions imply is NOT costed: Capex is blank on all 15 rows, so the life extension is free to the optimiser. Two VRE rows are the weakest case, 25 yr being a real design life rather than an inherited placeholder: Nakhchivan_Solar_PV 20 MW and Azerbaijan_Khizi_Wind 14 MW, 34 MW in total, are kept online on the assumption of repowering in place at zero capex. Replace any of these dates on sight if a published schedule appears. THIS COUNTRY: 4 rows, 1,069.0 MW kept online, across both zones. AzerbaijanMain: Azerbaijan_CHP_Legacy 985 MW (2029 to 2060) and Azerbaijan_Khizi_Wind 14 MW (2034 to 2060). Nakhchivan: Nakhchivan_GasTurbine_OCGT 50 MW and Nakhchivan_Solar_PV 20 MW (both 2040 to 2060). The CHP bridge asset was introduced to cover a 4,300 GWh CHP gap absent from GEM; retiring it in 2029 removed the cover without removing the gap. |
...
**Proxied from**: Turkiye (EPIAS national hourly shape)  
**Original source**: Proxy load profiles (Azerbaijan, Nakhchivan) — built by run_blacksea_data.py

> ⚠ **Needs review**: No AZ hourly load data. Replace with Azerenerji SCADA when available. CORRECTED 2026-09-19: the entry previously cited an ENTSO-E Turkish shape and a seasonal mean.


**Method**: PROXY Turkish EPIAS national hourly shape, year 2023, reduced to 28 representative days

| Period | Method | Notes |
|--------|--------|-------|
| 2024–2053 | `PROXY_Turkey` | Turkish national hourly load from EPIAS (ENTSO-E does not cover Turkiye), year 2023, reduced to 28 representative days (4 seasons x 7 daytypes). Same source shape for AzerbaijanMain and Nakhchivan. No Azerbaijani hourly load data available. Since 2026-09-19 the shape is divided by its own 2023 annual peak (tops at 0.99, was 0.93). Nakhchivan equals the EastAna rows; AzerbaijanMain carries a shape exponent on top, see the zone entry. |
...
**Method**: CAP LIFTED - 3 for all countries and all years, non-binding by design
...
| 2024-2053, cap lifted 2026-09-06 | `ASSUMPTION` | Set to 3 for every country and every year, which retires the constraint. This answers the question the 2026-08-30 entry above left open and told the reader to settle before publishing any allprojects result. WHY. eMaxAnnualExportShareEnergy and eMaxAnnualImportShareEnergy, base.gms:951-960, cap a country's annual EXTERNAL trade at its own annual demand times this value. On imports that is a defensible dependency limit. On exports it indexes transit on domestic consumption, which has no physical or economic basis for a corridor country: Georgia's job in the allprojects case is to move Caspian and Armenian energy to the EU, and the volume it can honestly carry is set by the line ratings and the export price, not by what Georgians consume. The Azerbaijan and Armenia feed into Georgia is INTERNAL and uncapped, so only the EU exit leg was throttled, which is an artefact of where the country boundary falls rather than a constraint on the corridor. WHERE IT WAS BINDING. Measured across all 14 scenarios of waves 1-3, both directions, every country and every year, exactly one case touched the cap: LC_AllProjects Georgia, pinned at ratio 1.000 for five consecutive years, 2036 21,905/21,905 GWh through 2040 24,092/24,092 GWh. The second highest anywhere is 0.88, LC_AllProjects_VeryLow Georgia 2036; LC_BSSC peaks at 0.50 and LC_AllProjects_CBAM at 0.38. Lifting the cap is therefore a no-op in 13 of 14 scenarios and the pre-2026-09-06 allprojects gain of -7,502 $m was a FLOOR, not an estimate. WHY 3 AND NOT A LARGER NUMBER. 3 is above the physical ceiling everywhere with margin, so the constraint can never bind again, while keeping the RHS coefficient range tight for the solver. Highest physically reachable ratio, external NTC times 8760 over national demand, over all years and both directions in allprojects: Georgia 2.28 on import and 2.16 on export, both in 2036, falling to 1.96 on export by 2040 as demand grows; Azerbaijan 0.24; Turkiye 0.03; Armenia has no external link at all. 3 clears the worst case by 30 percent. APPLIED UNIFORMLY, NOT AS A GEORGIA EXCEPTION. The 2026-08-30 entry floated a Georgia-specific value. One rule for all five countries is the honest form: the cap binds nowhere else, so a Georgia-only number would encode a special pleading that the numbers do not need. WHAT THIS IS NOT. It is not a claim that the corridor is unconstrained. External zones in EPM are infinite price-taking sinks, so with this cap gone the only limits on Georgian export are the NTC and pTradePriceExport. Results become MORE sensitive to the EU price assumption, which is what waves 2 (CBAM), 3 (verylow) and 5 (pricefan) exist to test. Any allprojects headline must be reported alongside those, never alone. MECHANICAL NOTE FOR ANYONE TEMPTED TO DELETE THE FILE INSTEAD. There is no default fill for this parameter anywhere in the code, and the two equations carry no dollar guard on its value, so a missing (y,c) entry is 0 in GAMS and forces that country's external trade to zero. Blanking cells or removing rows is the opposite of removing the constraint. A high value is the only way to neutralise it without editing base.gms. |

*Confidence: [HIGH] · Last updated: 2026-09-06*


<a id="azerbaijan-plossfactorinternal"></a>
...
| Period | Method | Notes |
|--------|--------|-------|
| 2024 to 2053 | `ASSUMPTION` | AzerbaijanMain to Armenia, Georgia and Nakhchivan, and Nakhchivan to EastAna (Turkiye): model values kept unchanged. No catalog source documents them. |
| 2032 to 2053, GECO scenarios | `CHANGED` | CHANGED 2026-09-11. GEC layer, read only by the five GECO scenarios through zcmap_geco.csv. AzerbaijanMain to GEC_AZ tap at a technical floor, because EPM rejects a zero loss. GEC_AZ to GEC_GE leg at the CESI bus-bar to bus-bar loss, converter stations included. Both directions. The leg loss is weighted by the GEC links in service (T5 p79, p81, p83). Values in cesi/cesi_register.yaml (k04). Source CESI GEC T5 p76, p79, p81 and p83. The other scenarios drop these rows through the zone filter. |

*Confidence: [MEDIUM] · Last updated: 2026-09-11*


<a id="azerbaijan-zcmap"></a>
...
| Period | Method | Notes |
|--------|--------|-------|
| 2032 to 2053, GECO scenarios | `CHANGED` | CHANGED 2026-09-11. GEC_AZ, the Azerbaijan RES hub of the CESI corridor, is a zone of its own mapped to Azerbaijan in zcmap_geco.csv, read only by the five GECO scenarios. It is isolated from the Azerbaijan grid and linked to it by a tap and to the GEC_GE node by the GEC leg only, as in the CESI topology. The hub candidates are documented under AzerbaijanMain pGenDataInput, the tap and the leg under pTransferLimit. Values in cesi/cesi_register.yaml (k17). Source CESI GEC T6 p34 Fig 5-11. The other scenarios read zcmap.csv and never see this zone. |
...
| no unplanned retirement, 2026-09-05 | `ASSUMPTION` | Every retirement date inside the 2025-2040 horizon that is not a published plan date is pushed beyond the horizon. WHY. The 2026-08-29 cleanup rewrote only the rows carrying the mechanical StYr + 25/30/50 placeholder and deliberately left the rest, so the model was still closing plants on inherited age assumptions rather than on any announced schedule: the Georgian gas fleet lost 693 MW between 2027 and 2036, the Azerbaijani CHP bridge 985 MW in 2029, and 2,256 MW of Turkish thermal left the fleet by 2038. None of those dates is traceable to an operator or government decommissioning plan. A retirement the plan does not contain is a modelling assumption that silently creates a capacity need and biases every investment and trade result built on it. RULE APPLIED (RetrYr and Life columns only): a row is touched if it sits in a modelled zone (zcmap.csv), has Status 1 or 2, and RetrYr is 2040 or earlier. It receives RetrYr = 2060 and Life = 2060 - StYr. 2060 is the floor the 2026-08-29 pass already used for hydro; it is beyond the horizon, so with fEnableEconomicRetirement = 0 main.gms:832 pins vCap at nameplate for every horizon year and main.gms:814 never fires. Life is kept arithmetically coherent with RetrYr = StYr + Life so the two columns stay auditable; on these rows it is a bookkeeping value, NOT a technical life estimate, and it is inert for cost because Capex is blank on every row touched, so pCRF multiplies a zero capex. 15 rows across four countries. Storage (pStorageDataInput) has no row retiring inside the horizon and is untouched, and no Status 3 candidate retires inside the horizon. KEPT: Armenia_ANPP 407.5 MW retires 2036, a published Rosatom life-extension date, with Armenia_Nuclear_Cand standing as its successor candidate. WHAT THIS IS NOT. It is not a claim that these plants will run to 2060. It is the statement that the study does not close a plant the plan does not close, and that the horizon end, not an assumed age, is where the fleet is cut. The refurbishment those extensions imply is NOT costed: Capex is blank on all 15 rows, so the life extension is free to the optimiser. Two VRE rows are the weakest case, 25 yr being a real design life rather than an inherited placeholder: Nakhchivan_Solar_PV 20 MW and Azerbaijan_Khizi_Wind 14 MW, 34 MW in total, are kept online on the assumption of repowering in place at zero capex. Replace any of these dates on sight if a published schedule appears. THIS ZONE: 2 rows, 999.0 MW kept online. Azerbaijan_CHP_Legacy 985 MW (2029 to 2060) and Azerbaijan_Khizi_Wind 14 MW (2034 to 2060). See the same entry under the Azerbaijan country section for the full rule. |
| 2032 to 2053, GECO scenarios | `CHANGED` | CHANGED 2026-09-11. Azerbaijan RES hub of the CESI study, read only by the five GECO scenarios through zcmap_geco.csv. Three candidates in the GEC_AZ zone, PV, onshore wind and offshore wind, capped at the CESI hub capacity by 2040. PV and onshore from the first GEC link, offshore from the second, as in the CESI schedule. The yearly build limit equals the cap, so the model chooses the pace within the cap and the corridor capacity. Costs, availability and VRE profiles are those of AzerbaijanMain, copied under GEC_AZ in the default files and pVREProfile, not taken from CESI. Values in cesi/cesi_register.yaml (a07). Source CESI GEC T6 p26 Fig 5-2. The other scenarios drop these rows through the zone filter. |
...
> ⚠ **Needs review**: PROXY: no Azerbaijani hourly load data. Turkish shape used as proxy (similar climate: continental, hot summers, cold winters), bent to the forecast load factor. Replace with Azerenerji SCADA when available. CORRECTED 2026-09-19: the entry previously cited an ENTSO-E Turkish shape and a seasonal mean.
...
| Period | Method | Notes |
|--------|--------|-------|
| 2024–2053 | `PROXY_Turkey` | Load shape: Turkish national hourly load from EPIAS (ENTSO-E does not cover Turkiye), built by run_blacksea_data.py, year 2023, reduced to 28 representative days (4 seasons x 7 daytypes) by the regional representative days pipeline. |
| normalisation and shape, 2026-09-19 | `PROXY_Turkey` | Redeployed by deploy_repdays.py --demand-only --adjust-shape Armenia AzerbaijanMain. Same 28 days and weights. The Turkish shape is divided by its own 2023 annual peak (tops at 0.99, was 0.93 on the 2018 to 2024 maximum). Then a shape exponent is applied, p = pmax x (p / pmax)^1.447, found by bisection so that the profile load factor equals the mean forecast load factor (0.600, against 0.697 for the Turkish shape). The peak is unchanged; the lowest value is 0.334. The rows therefore no longer equal the EastAna rows. The residual energy gap spread by the simplified demand routine falls to about 2 percent in 2030 (18 percent without the exponent). Previous file kept as load/pDemandProfile.csv.bak_repdays_20260919. |
...
<a id="azerbaijanmain-pvreprofile"></a>
...
| no unplanned retirement, 2026-09-05 | `ASSUMPTION` | Every retirement date inside the 2025-2040 horizon that is not a published plan date is pushed beyond the horizon. WHY. The 2026-08-29 cleanup rewrote only the rows carrying the mechanical StYr + 25/30/50 placeholder and deliberately left the rest, so the model was still closing plants on inherited age assumptions rather than on any announced schedule: the Georgian gas fleet lost 693 MW between 2027 and 2036, the Azerbaijani CHP bridge 985 MW in 2029, and 2,256 MW of Turkish thermal left the fleet by 2038. None of those dates is traceable to an operator or government decommissioning plan. A retirement the plan does not contain is a modelling assumption that silently creates a capacity need and biases every investment and trade result built on it. RULE APPLIED (RetrYr and Life columns only): a row is touched if it sits in a modelled zone (zcmap.csv), has Status 1 or 2, and RetrYr is 2040 or earlier. It receives RetrYr = 2060 and Life = 2060 - StYr. 2060 is the floor the 2026-08-29 pass already used for hydro; it is beyond the horizon, so with fEnableEconomicRetirement = 0 main.gms:832 pins vCap at nameplate for every horizon year and main.gms:814 never fires. Life is kept arithmetically coherent with RetrYr = StYr + Life so the two columns stay auditable; on these rows it is a bookkeeping value, NOT a technical life estimate, and it is inert for cost because Capex is blank on every row touched, so pCRF multiplies a zero capex. 15 rows across four countries. Storage (pStorageDataInput) has no row retiring inside the horizon and is untouched, and no Status 3 candidate retires inside the horizon. KEPT: Armenia_ANPP 407.5 MW retires 2036, a published Rosatom life-extension date, with Armenia_Nuclear_Cand standing as its successor candidate. WHAT THIS IS NOT. It is not a claim that these plants will run to 2060. It is the statement that the study does not close a plant the plan does not close, and that the horizon end, not an assumed age, is where the fleet is cut. The refurbishment those extensions imply is NOT costed: Capex is blank on all 15 rows, so the life extension is free to the optimiser. Two VRE rows are the weakest case, 25 yr being a real design life rather than an inherited placeholder: Nakhchivan_Solar_PV 20 MW and Azerbaijan_Khizi_Wind 14 MW, 34 MW in total, are kept online on the assumption of repowering in place at zero capex. Replace any of these dates on sight if a published schedule appears. THIS ZONE: 2 rows, 70.0 MW kept online. Nakhchivan_GasTurbine_OCGT 50 MW and Nakhchivan_Solar_PV 20 MW, both 2040 to 2060. See the same entry under the Azerbaijan country section for the full rule. The review_note below on the OCGT retirement year is answered by this entry. |
...
**Method**: PROXY from AzerbaijanMain load shape (same Turkish EPIAS origin)

| Period | Method | Notes |
|--------|--------|-------|
| 2024–2053 | `PROXY_AzerbaijanMain` | Turkish EPIAS shape on 28 representative days (4 seasons x 7 daytypes), equal to the EastAna rows. Nakhchivan has similar climate to southern AZ (continental, hot summers). No Nakhchivan-specific hourly load data available. Since 2026-09-19 the shape is divided by its own 2023 annual peak (tops at 0.99, was 0.93). No shape exponent: the forecast load factor is within 0.05 of the profile, so the rows differ from AzerbaijanMain, which carries one. |
...
> ⚠ **Needs review**: Since 2026-09-19 the profile is divided by its own 2023 annual peak (tops at 0.97, was 0.86 on the 2018 to 2024 maximum). Open point: the load factor of the profile (0.72) is well above the forecast one (0.58), so the simplified demand routine would remove about 23 percent of the energy from the off-peak hours. No shape exponent applied, because Romania is in no EPM run (no scenario uses zcmap_robg.csv). Settle it before any run that includes Romania.
...
| Period | Method | Notes |
|--------|--------|-------|
| 2024–2053 | `DIRECT` | ENTSO-E Actual Total Load, RO bidding zone, downloaded by run_blacksea_data.py. Year 2023 reduced to 28 representative days (4 seasons x 7 daytypes) by the regional representative days pipeline. Verified 2026-09-19: 28 distinct rows, d1 to d7. CORRECTED 2026-09-19: the entry previously described a seasonal mean over d1 to d6. |

*Confidence: [HIGH] · Last updated: 2026-09-19*
...
| Period | Method | Notes |
|--------|--------|-------|
| 2024–2053 | `DIRECT` | ENTSO-E Actual Total Load, BG bidding zone, downloaded by run_blacksea_data.py. Year 2023 reduced to 28 representative days (4 seasons x 7 daytypes) by the regional representative days pipeline. Verified 2026-09-19: 28 distinct rows, d1 to d7. CORRECTED 2026-09-19: the entry previously described a seasonal mean over d1 to d6. |
