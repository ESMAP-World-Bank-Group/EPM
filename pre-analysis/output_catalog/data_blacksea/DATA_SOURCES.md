...
| Period | Method | Notes |
|--------|--------|-------|
| 2037 to 2053, LC_ArmNuke | `CHANGED` | CHANGED 2026-09-15. Forced successor to the Metsamor plant, read only by LC_ArmNuke through supply/pGenDataInput_armnuke.csv. That file is supply/pGenDataInput.csv with one row changed: Armenia_Nuclear_Cand goes from Status 3 (candidate) to Status 2 (committed), StYr 2036 to 2037 and RetrYr 2086 to 2087 (Life 50 kept). 2037 is the first model year without Armenia_ANPP (407.5 MW, RetrYr 2036), so the two plants never run in the same year. A committed plant is built for its full Capacity, 300 MW, at StYr (main.gms:728) and pays the candidate Capex, 8 $m/MW flat (the Armenia Nuclear capex trajectory is all zero, so the plant sits outside dc). LC_ArmNuke is LC_Baseline otherwise, the Georgia to Armenia line included. In every other scenario the candidate stays optional at StYr 2036 and is never built. |
...
| 2024 to 2053, LC_Baseline_NoArGe | `CHANGED` | CHANGED 2026-09-15. Sensitivity without the existing Georgia to Armenia line, read only by LC_Baseline_NoArGe through trade/pTransferLimit_noarge.csv. That file is trade/pTransferLimit.csv with the eight Armenia to Georgia and Georgia to Armenia rows (Q1 to Q4, both directions) set from 150 MW to 0 MW in every year; every other row is identical. The pair stays in sTopology and the Georgia to Armenia row of pNewTransmission_none.csv keeps MaximumNumOfLines 0, so nothing can be built on it. LC_Baseline and every other scenario keep the line at 150 MW both ways. |
