...
def _plot_feature_heatmap(
    ...
    im = ax.imshow(data.values, aspect="auto", cmap="viridis_r", vmin=0, vmax=1)
    ax.set_xticks(range(len(months)))
    ax.set_xticklabels(month_labels, rotation=0, ha="center")
    ax.set_yticks(range(len(feature_labels)))
    ax.set_yticklabels(feature_labels)
    ax.set_title("Monthly mean features (normalized by annual max)")
...
def _plot_season_means_heatmap(
    ...
    im = ax.imshow(df.values.T, aspect="auto", cmap="viridis_r", vmin=0, vmax=1)
    ...
    cbar = fig.colorbar(im, ax=ax)
    cbar.ax.set_ylabel("Mean normalized value (dark = higher)")
    fig.tight_layout()
    fig.savefig(path, dpi=200)
    if show:
        plt.show()
